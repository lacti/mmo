                ...
                if (window) {
                    for (i = 0; i < state->num_windows; ++i) {
                        if (window == state->windows[i] &&
                            ...
                            rs_free_backbuffer();
                            rs_create_backbuffer(viewport.w, viewport.h);

                            if (gContext)
                            {
                                gContext->SetDimensions(Rocket::Core::Vector2i(viewport.w, viewport.h));
                            }
                        }
                    }
                }
    ...
    case SDL_KEYDOWN:
        ...
        case SDLK_g:
            if (event->key.keysym.mod & KMOD_CTRL) {
                ...
                SDL_Window *window = SDL_GetWindowFromID(event->key.windowID);
                if (window) {
                    SDL_SetWindowGrab(window, SDL_GetWindowGrab(window) ? SDL_FALSE : SDL_TRUE);
                }
            }
            break;
        case SDLK_m:
        ...
        case SDLK_ESCAPE:
            {
                if (!gContext->GetDocument("escmenu"))
                {
                    EventManager::LoadWindow("escmenu");
                }
